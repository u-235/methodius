 ...
 */
public class PixselMapX extends Object
{
    ...
    /**
     * Итератор для последовательного доступа к пикселям {@linkplain PixselMapX
    ...
    public class PixselIterator
    {
         ...
         * @param h
         */
        protected PixselIterator(PixselMapX src, int x, int y, int w, int h,
                        int dir) {
            this.map = src;
            this.dir = dir;
            startX = x;
        ...
        public void setNext(boolean set) {
            if (!hasNext()) throw new BadIterationException();
            // TODO Auto-generated method stub
     ...
     */
    public PixselMapX(int width, int height) {
        init(width, height);
    }
     ...
     */
    public PixselMapX(int width, int height, boolean[] src) {
        init(width, height);
     ...
     */
    public PixselMapX(int width, int height, byte[] src) {
        init(width, height);
        fromArray(src, pixsels);
    }
     ...
     * @see #copy(PixselMapX)
     */
    public PixselMapX(PixselMapX src) {
     ...
     * @see #clone()
     */
    public void copy(PixselMapX src) {
        byte[] tm;
     ...
     * обёрткой для {@link #PixselMapX(PixselMapX)}.
     * 
     * @return Копия карты.
     * @see #PixselMapX(PixselMapX)
     * @see #copy(PixselMapX)
     */
    @Override
    public PixselMapX clone() {
        return new PixselMapX(this);
    }
    ...
    public boolean equals(Object s) {
        ...
        if (!(s instanceof PixselMapX)) return false;

        return equals((PixselMapX) s);
    }
     ...
     * @return <b>true</b> если символы равны.
     */
    public boolean equals(PixselMapX s) {
        int i;
     ...
     * @return
     */
    public PixselMapX getFragment(int x, int y, int w, int h) {
        if (x < 0) {
            w -= x;
            x = 0;
        }
        ...
        PixselMapX rv = new PixselMapX(w, h);
     ...
     * @param fragment
     */
    public void setFragment(int x, int y, PixselMapX fragment) {
        int w;
        int h;
    ...
    public PixselMapX getDifferent(PixselMapX old) {
        PixselMapX rv;
        int w = 0;
        int h = 0;
        ...
        if (this.width != old.width || this.height != old.height) {
            rv = new PixselMapX(old);
            rv.diff = true;
            return rv;
        }

        rv = new PixselMapX(w, h);

        return rv;
    }

    public void restore(PixselMapX different) {
    ...
    protected static boolean copyFrame(PixselMapX src, int srcX, int srcY,
                    PixselMapX dst, int dstX, int dstY, int w, int h) {
